...
class Arc(object):
    ...
    def calFlagForDeg3(self):
        # Respectively calculate the angle between the two edges and the positive half axis of the x-axis
        ...
        angle1 = math.atan2(dy1, dx1) # radian
        angle1 = int(angle1 * 180/math.pi) # radian -> degree
        angle2 = math.atan2(dy2, dx2)
        angle2 = int(angle2 * 180/math.pi)
        included_angle = 0 # angle between the two edges, only include < 180 degree

        # default: the two edges are at the same side. So edges with larger angle is pre-0.
        self.deg3StartFlag = 0 if angle1 > angle2 else 1
        self.deg3EndFlag = 0 if angle1 <= angle2 else 1

        # if the two edges are not at the same side
        if angle1*angle2 < 0:
    ...
    def calClockwiseAngle(self, dx1, dy1, dx2, dy2):
        dot = dx1 * dx2 + dy1 * dy2
        det = dx1 * dy2 - dy1 * dx2
    ...
    def distributeDeg2Vertices(self, chain):
        ...
        if leftInterval > 0 and index < len(chain) - 1: # only one step, rotate arc length is leftInterval
            ...
            if self.deg3StartFlag == 0: # ccw
                chain[index].x, chain[index].y = self.calDividePointCCW(tangencyEndCircleCenter_x, tangencyEndCircleCenter_y, tangencyPointEnd_x, tangencyPointEnd_y, angle)
                index += 1
            else:    # cw
                chain[index].x, chain[index].y = self.calDividePointCW(tangencyEndCircleCenter_x, tangencyEndCircleCenter_y, tangencyPointEnd_x, tangencyPointEnd_y, angle)
                index += 1
    ...
    def calTangencyCircleOutside(self, chain, centroid, radius):
        ...
        firstDeg2Edge = chain[1].incidentEdge
        if "i" != firstDeg2Edge.incidentFace.identifier:
            firstDeg2Edge = firstDeg2Edge.twin

        firstDeg2EdgeOriginId = firstDeg2Edge.origin.identifier
        nextVertexId = firstDeg2Edge.next.origin.identifier
        if nextVertexId == chain[0].identifier and firstDeg2EdgeOriginId == chain[1].identifier or nextVertexId == chain[1].identifier and firstDeg2EdgeOriginId == chain[2].identifier:
            self.deg3StartFlag = 1
            self.deg3EndFlag = 0
        else:
            self.deg3StartFlag = 0
            self.deg3EndFlag = 1
        ...
        if self.deg3StartFlag == 0:
            if isTargetCirclePre:
                self.startCircleTangency = circle(circle2_x, circle2_y, radius2, 0)
            else:
                self.startCircleTangency = circle(circle1_x, circle1_y, radius2, 0)
        else:
        ...
        if self.deg3EndFlag == 0:
            if isTargetCirclePre:
                self.endCircleTangency = circle(circle4_x, circle4_y, radius1, 0)
            else:
                self.endCircleTangency = circle(circle3_x, circle3_y, radius1, 0)
        else:
            if not isTargetCirclePre:
                self.endCircleTangency = circle(circle4_x, circle4_y, radius1, 0)
            else:
    ...
    def distributeDeg2VerticesOutside(self, chain, centroid):
        ...
        dy1 = tangencyPointStart_y - centroid.y if self.deg3StartFlag == 1 else tangencyPointEnd_y - centroid.y
        dx2 = tangencyPointStart_x - centroid.x if self.deg3StartFlag != 1 else tangencyPointEnd_x - centroid.x
        dy2 = tangencyPointStart_y - centroid.y if self.deg3StartFlag != 1 else tangencyPointEnd_y - centroid.y
        chordRadian = self.calClockwiseAngle(dx1, dy1, dx2, dy2)
        apollonisArcLength = self.apollonisCircle.radius * chordRadian

        startArcLength = self.calArcLength(self.deg3Start.x, tangencyPointStart_x, self.deg3Start.y, tangencyPointStart_y, self.startCircleTangency.radius)
        ...
        while leftLength >= interval: # still on first arc
            # Calculate the position of the movement, cw or ccw
            angle = interval / self.startCircleTangency.radius
            if self.deg3StartFlag == 0:   # ccw
                chain[index].x, chain[index].y = self.calDividePointCCW(self.startCircleTangency.center_x, self.startCircleTangency.center_y, chain[index-1].x, chain[index-1].y, angle)
                index += 1
            else:   # clockwise
                chain[index].x, chain[index].y = self.calDividePointCW(self.startCircleTangency.center_x, self.startCircleTangency.center_y, chain[index-1].x, chain[index-1].y, angle)
                index += 1
            leftLength = leftLength - interval
        ...
        if leftInterval > 0 and index < len(chain) - 1: # only one step, rotate arc length is leftInterval
            # Calculate the position of the movement, cw or ccw
            angle = leftInterval / self.endCircleTangency.radius
            if self.deg3StartFlag == 0: # ccw
                chain[index].x, chain[index].y = self.calDividePointCCW(self.endCircleTangency.center_x, self.endCircleTangency.center_y, tangencyPointEnd_x, tangencyPointEnd_y, angle)
                index += 1
            else:    # cw
                chain[index].x, chain[index].y = self.calDividePointCW(self.endCircleTangency.center_x, self.endCircleTangency.center_y, tangencyPointEnd_x, tangencyPointEnd_y, angle)
                index += 1
        leftLength = endArcLength - leftInterval

        while leftLength > interval and index < len(chain) - 1:
            # Calculate the position of the movement, cw or ccw
            angle = interval / self.endCircleTangency.radius
            if self.deg3StartFlag == 0: # ccw
                chain[index].x, chain[index].y = self.calDividePointCCW(self.endCircleTangency.center_x, self.endCircleTangency.center_y, chain[index-1].x, chain[index-1].y, angle)
                # index += 1
            else: # cw
                chain[index].x, chain[index].y = self.calDividePointCW(self.endCircleTangency.center_x, self.endCircleTangency.center_y, chain[index-1].x, chain[index-1].y, angle)
                # index += 1
            leftLength = leftLength - interval
        # leftInterval = interval - leftLength # should be 0
        ############################
