...
class Arc(object):
    ...
    def calTangencyCircleOutside(self, centroid, radius):
        ...
        # TODO fomula https://blog.csdn.net/zx3517288/article/details/53326420
        d = math.sqrt((self.deg3Start.x - centroid.x)**2 + (self.deg3Start.y - centroid.y)**2)
        a = (radius^2 - (2*radius)^2 + d^2) / (2*d)
        h = math.sqrt((2*radius)^2 - (d - a)^2)
        x0 = self.deg3Start.x + a/d * (centroid.x - self.deg3Start.x)
        y0 = self.deg3Start.y + a/d * (centroid.y - self.deg3Start.y)
        circle1_x = x0 - h/d * (centroid.y - self.deg3Start.y)
        circle1_y = y0 + h/d * (centroid.x - self.deg3Start.x)
        circle2_x = x0 + h/d * (centroid.y - self.deg3Start.y)
        circle2_y = y0 - h/d * (centroid.x - self.deg3Start.x)
    ...
    def distributeOutsideDeg2Vertices(self, chain, centroid, radius):
        ...
        self.tangencyPointStart[0] = (self.startCircleTangency.center_x + centroid.x) / 2
        self.tangencyPointStart[1] = (self.startCircleTangency.center_y + centroid.y) / 2
        self.tangencyPointEnd[0] = (self.endCircleTangency.center_x + centroid.x) / 2
        self.tangencyPointEnd[1] = (self.endCircleTangency.center_y + centroid.y) / 2
