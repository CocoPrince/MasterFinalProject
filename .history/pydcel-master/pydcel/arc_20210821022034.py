...
class Arc(object):
    ...
    '''outside chain'''
    def calTangencyCircleOutside(self, face_cur_centroid, face_other_centroid, radius_cur, radius_other):
        # apollo圆的圆心和半径
        radius, center_x, center_y = self.calApollonisCircleInfo(self, face_cur_centroid, face_other_centroid, radius_cur, radius_other)
        # TODO推公式
        self.startCircleTangency = circle(x1, y1, radius, 0)
        self.endCircleTangency = circle(x2, y2, radius, 0)

        # TODO 用夹角选合适的第1、3段弧的圆
        radian = self.calRandianBetween2Vector(self, kCentroid, centroid1, centroid2)
