...
class dcelVis(Tk):
    ...
    #     options = {'size':5, 'fill':'red', 'outline':''}
    #     count = 1
    #     for v, radius in zip(self.D.apollonisCenterList, self.D.apollonisRadiusList):
    ...
    def draw_dcel_text(self, v, size,  **options):
        return self.draw.text(v.x, v.y, size, **options)

    def draw_dcel_id_text(self, v, **options):
        return self.draw.id_text(v.x, v.y, **options)

    def draw_dcel_centroid_edge(self, v0, v1, **options):
        if options == {}:
            options = {'arrow':NONE, 'arrowshape':(7,6,2), 'fill': 'orange'}
        return self.draw.edge((v0.x, v0.y), (v1.x, v1.y) , **options)

    def draw_dcel_vertex(self, v, **options):
        if options == {}:
            options = {'size':3, 'fill':'blue', 'outline':''}
    ...
    def draw_dcel_hedge(self, e, **options):
        if options == {}:
            options = {'arrow':NONE, 'arrowshape':(7,6,2), 'fill': '#444444'}
