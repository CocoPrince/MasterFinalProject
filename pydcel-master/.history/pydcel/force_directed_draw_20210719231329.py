...
class force_directed(object):
    ...
    def handle3DegVertex(self, threeDimVertex, centroidList):
        flag = True
        total = 0.0
        while flag:
            xDisRepulsive, yDisRepulsive = self.cal3DimRepulsiveForce( threeDimVertex, centroidList)
            xDisAttractive, yDisAttractive = self.cal3DimAttractiveForce(threeDimVertex, centroidList)
            for centroid in centroidList:
