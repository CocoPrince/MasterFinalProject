    ...
    def handleRotateRepusive(self):
        for kCentroid, vEdgeList in self.centroidEdgeDict.items():
            ...
            while index < len(vEdgeList):
                ...
                self.xDisDit[postCentroid.identifier] = self.xDisDit[postCentroid.identifier] + 0.01*((postCentroid.x-kCentroid.x)*math.cos(angle) + (postCentroid.y-kCentroid.y)*math.sin(angle)+kCentroid.x - postCentroid.x)
                self.yDisDit[postCentroid.identifier] = self.yDisDit[postCentroid.identifier] + 0.01*((postCentroid.y-kCentroid.y)*math.cos(angle) - (postCentroid.x-kCentroid.x)*math.sin(angle)+kCentroid.y - postCentroid.y)
