...
class force_directed(object):
    ...
    def rotateRepulsiveForce(self, idealRadian, realRadian):
        ...
        return - idealRadian / 3000*realRadian 
    ...
    def handleRotateRepusive(self):
        for kCentroid, vEdgeList in self.centroidEdgeDict.items():
            ...
            while index < len(vEdgeList):
                ...
                x = (postCentroid.x-kCentroid.x)*math.cos(angle) + (postCentroid.y-kCentroid.y)*math.sin(angle)+kCentroid.x
                y = (postCentroid.y-kCentroid.y)*math.cos(angle) - (postCentroid.x-kCentroid.x)*math.sin(angle)+kCentroid.y

                self.xDisDit[postCentroid.identifier] = self.xDisDit[postCentroid.identifier] + (x - postCentroid.x)
                self.yDisDit[postCentroid.identifier] = self.yDisDit[postCentroid.identifier] + (y - postCentroid.y)
    ...
    def rotateRepulsive(self, realRadian, chainBetweenKCentroidAndPre, chainBetweenKCentroidAndPost, vertexListOfKCentroid):       
        ...
        idealRadian = count / len(vertexListOfKCentroid) * (360 / 2 * math.pi)
        ...
        # return (realRadian - idealRadian) / 50 # TODO done. positive or negative sign
