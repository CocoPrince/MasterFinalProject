...
class vertex(object):   
    def __init__(self, px, py, pz, identifier):
        ...
        self.faces = set()   # all faces that are incident to the vertex
...
class DCEL(object):
    ...
    def calCentroidAndRadius(self, face_vertices):
        ...
        stand_radius = math.pow(optimal_area/math.pi, 1/2) * 0.9
    ...
    def getAllIncidentEdge(self, vertex, edge_dict): 
        edge_list = []
        edge_ientifier_set = vertex.incidentEdges
        for edge_identifier in edge_ientifier_set:               
    ...
    def getDestinationVerticesOnCircle(self, face_dict, vertex):
        ...
        distinct_set = set()
        for incident_face_id in vertex.faces:
    ...
    def handleFaces(self, isHandle):
        ...
        for repeat in range(1):
            ...
            edges = []
            distinct_edge = set()
            face_dict = {} #存储通过id查找面的字典
            for face in self.faceList:
                face_dict[face.identifier] = face
            edge_dict = {} #存储通过id查找hedge的字典
            ...
            for vertex in self.vertexList:
                edge_list, edge_ientifier_set = self.getAllIncidentEdge(vertex, edge_dict)
