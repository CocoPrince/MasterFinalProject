...
class force_directed(object):
    ...
    def calAttractiveForce(self):
        for edge in self.edges:
            ...
            self.xDisDit[startcentroid.identifier] = self.xDisDit[startcentroid.identifier] - distX / dist * self.attractiveForce(idealDis, dist) * dist / abs(dist)
            self.yDisDit[startcentroid.identifier] = self.yDisDit[startcentroid.identifier] - distY / dist * self.attractiveForce(idealDis, dist) * dist / abs(dist)
            self.xDisDit[endcentroid.identifier] = self.xDisDit[endcentroid.identifier] + distX / dist * self.attractiveForce(idealDis, dist) * dist / abs(dist)
