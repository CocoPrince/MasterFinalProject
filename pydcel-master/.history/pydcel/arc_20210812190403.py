    ...
    def distributeDeg2Vertices(self, chain):
        ...
        while leftLength >= interval: # still on first arc
            ...
            if self.deg3StartFlag == 0:   # ccw
                chain[index].x = tangencyStartCircleCenter_x + math.cos(angle) * (self.deg3Start.x - tangencyStartCircleCenter_x) - math.sin(angle) * (self.deg3Start.y - tangencyStartCircleCenter_y)
                chain[index].y = tangencyStartCircleCenter_y + math.sin(angle) * (self.deg3Start.x - tangencyStartCircleCenter_x) + math.cos(angle) * (self.deg3Start.y - tangencyStartCircleCenter_y)
                index += 1
            else:   # clockwise
                chain[index].x = tangencyStartCircleCenter_x + math.cos(angle) * (self.deg3Start.x - tangencyStartCircleCenter_x) + math.sin(angle) * (self.deg3Start.y - tangencyStartCircleCenter_y)
                chain[index].y = tangencyStartCircleCenter_y - math.sin(angle) * (self.deg3Start.x - tangencyStartCircleCenter_x) + math.cos(angle) * (self.deg3Start.y - tangencyStartCircleCenter_y)
        ...
        if leftInterval > 0: # only one step, rotate arc length is leftInterval
            ...
            if self.deg3StartFlag == 0:  # cw
                chain[index].x = self.apollonisCircle.center_x + math.cos(angle) * (start切点.x - self.apollonisCircle.center_x) + math.sin(angle) * (strat切点.y - self.apollonisCircle.center_y)
                chain[index].y = self.apollonisCircle.center_y - math.sin(angle) * (start切点.x - self.apollonisCircle.center_x) + math.cos(angle) * (strat切点.y - self.apollonisCircle.center_y)
                index += 1
            else:   # ccw
                chain[index].x = self.apollonisCircle.center_x + math.cos(angle) * (start切点.x - self.apollonisCircle.center_x) - math.sin(angle) * (strat切点.y - self.apollonisCircle.center_y)
                chain[index].y = self.apollonisCircle.center_y + math.sin(angle) * (start切点.x - self.apollonisCircle.center_x) + math.cos(angle) * (strat切点.y - self.apollonisCircle.center_y)
        ...
        while leftLength >= interval: # on second arc
            ...
            angle = interval / math.pi * 2
            if self.deg3StartFlag == 0: # cw
                chain[index].x = self.apollonisCircle.center_x + math.cos(angle) * (chain[index].x - self.apollonisCircle.center_x) + math.sin(angle) * (chain[index].y - self.apollonisCircle.center_y)
                chain[index].y = self.apollonisCircle.center_y - math.sin(angle) * (chain[index].x - self.apollonisCircle.center_x) + math.cos(angle) * (chain[index].y - self.apollonisCircle.center_y)
                index += 1
            else:  # ccw
                chain[index].x = self.apollonisCircle.center_x + math.cos(angle) * (chain[index].x - self.apollonisCircle.center_x) - math.sin(angle) * (chain[index].y - self.apollonisCircle.center_y)
                chain[index].y = self.apollonisCircle.center_y + math.sin(angle) * (chain[index].x - self.apollonisCircle.center_x) + math.cos(angle) * (chain[index].y - self.apollonisCircle.center_y)
        ...
        if leftInterval > 0: # only one step, rotate arc length is leftInterval
            ...
            angle = leftInterval / math.pi * 2
            if self.deg3StartFlag == 0: # ccw
                chain[index].x = tangencyEndCircleCenter_x + math.cos(angle) * (end切点.x - tangencyEndCircleCenter_x) - math.sin(angle) * (end切点.y - tangencyEndCircleCenter_y)
                chain[index].y = tangencyEndCircleCenter_y + math.sin(angle) * (end切点.x - tangencyEndCircleCenter_x) + math.cos(angle) * (end切点.y - tangencyEndCircleCenter_y)
                index += 1
            else:    # cw
                chain[index].x = tangencyEndCircleCenter_x + math.cos(angle) * (end切点.x - tangencyEndCircleCenter_x) + math.sin(angle) * (end切点.y - tangencyEndCircleCenter_y)
                chain[index].y = tangencyEndCircleCenter_y - math.sin(angle) * (end切点.x - tangencyEndCircleCenter_x) + math.cos(angle) * (end切点.y - tangencyEndCircleCenter_y)
        ...
        while leftLength >= interval:
            ...
            angle = interval / math.pi * 2
            if self.deg3StartFlag == 0: # ccw
                chain[index].x = tangencyEndCircleCenter_x + math.cos(angle) * (chain[index].x - tangencyEndCircleCenter_x) - math.sin(angle) * (chain[index].y - tangencyEndCircleCenter_y)
                chain[index].y = tangencyEndCircleCenter_y + math.sin(angle) * (chain[index].x - tangencyEndCircleCenter_x) + math.cos(angle) * (chain[index].y - tangencyEndCircleCenter_y)
                index += 1
            else: # cw
                chain[index].x = tangencyEndCircleCenter_x + math.cos(angle) * (chain[index].x - tangencyEndCircleCenter_x) + math.sin(angle) * (chain[index].y - tangencyEndCircleCenter_y)
                chain[index].y = tangencyEndCircleCenter_y - math.sin(angle) * (chain[index].x - tangencyEndCircleCenter_x) + math.cos(angle) * (chain[index].y - tangencyEndCircleCenter_y)
