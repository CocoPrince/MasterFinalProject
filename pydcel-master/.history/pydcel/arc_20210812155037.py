...
class Arc(object):
    ...
    def calTangencyCircle(self):
        ...
        if self.deg3StartFlag == 0:
            # clockwise
            tangencyStartCircleCenter_x = self.appoloCircle.center_x + math.cos(alpha) * (self.deg3Start.x - self.appoloCircle.center_x) + math.sin(alpha) * (self.deg3Start.y - self.appoloCircle.center_y)
            tangencyStartCircleCenter_y = self.appoloCircle.center_y - math.sin(alpha) * (self.deg3Start.x - self.appoloCircle.center_x) + math.cos(alpha) * (self.deg3Start.y - self.appoloCircle.center_y)
            # ccw
            tangencyEndCircleCenter_x = self.appoloCircle.center_x + math.cos(beta) * (self.deg3End.x - self.appoloCircle.center_x) - math.sin(beta) * (self.deg3End.y - self.appoloCircle.center_y)
            tangencyEndCircleCenter_y = self.appoloCircle.center_y + math.sin(beta) * (self.deg3End.x - self.appoloCircle.center_x) + math.cos(beta) * (self.deg3End.y - self.appoloCircle.center_y)
        else:
            # ccw
            tangencyStartCircleCenter_x = self.appoloCircle.center_x + math.cos(alpha) * (self.deg3Start.x - self.appoloCircle.center_x) - math.sin(alpha) * (self.deg3Start.y - self.appoloCircle.center_y)
            tangencyStartCircleCenter_y = self.appoloCircle.center_y + math.sin(alpha) * (self.deg3Start.x - self.appoloCircle.center_x) + math.cos(alpha) * (self.deg3Start.y - self.appoloCircle.center_y)
        ...
        for deg2 in chain:
            ...
            if leftInterval > 0:
                ...
                if startFlag == 0:
                    ...
                    pass
                leftLength = appoloArcLength - leftInterval

            while appoloArcLength >= interval:
                # 计算移动的位置,顺逆时针
                ...
                if startFlag == 0:
                    ...
                    pass
                leftLength = appoloArcLength - interval
            leftInterval = interval - leftLength
