class draw(object):
    ...
    def circle(self, x, y, size=3, **attributes):
        x,y = self.m.t(x,y)
        size = self.m.t_radius(size)
        return self.m.canvas.create_oval(x-size, y-size, x+size, y+size, **attributes)

    def text(self, x, y, size, **attributes):
        x,y = self.m.t(x,y)
        return self.m.canvas.create_text(0 + 100, 0 + size, **attributes)

    def id_text(self, x, y, size=7, **attributes):
        x,y = self.m.t(x,y)
        return self.m.canvas.create_text(x + size, y + size, **attributes)
